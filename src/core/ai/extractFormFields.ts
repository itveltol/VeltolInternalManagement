...
function buildFilePrompt(
  ...
  const fieldList = targetFields
    .map((f) => `- ${f}: ${descriptions[f] ?? FIELD_DESCRIPTIONS[f] ?? f}`)
    .join("\n");
...
/**
 * Resolves a supported MIME type for a file, falling back to its extension
 * when the source only reports a generic type (e.g. the local-folder dev
 * fallback, which always says application/octet-stream). Returns null when
 * the file can't be sent to the model.
 */
export function resolveMimeType(name: string, mimeType: string): string | null {
  if (MIME_TYPES.has(mimeType)) return mimeType;
  const ext = name.split(".").pop()?.toLowerCase() ?? "";
  return EXTENSION_MIME_TYPES[ext] ?? null;
}

interface ExtractFromFileInput {
  content: ArrayBuffer;
  name: string;
  /** Must be one of MIME_TYPES — see resolveMimeType. */
  mimeType: string;
  formType: FormType;
  targetFields: string[];
  context: Record<string, string>;
  locale: Locale;
  /** Per-call overrides for FIELD_DESCRIPTIONS (e.g. stricter formats). */
  descriptions?: Record<string, string>;
}

/** Extracts form field values from a document or image. */
export async function extractFieldsFromFile({
  ...
  locale,
  descriptions,
}: ExtractFromFileInput): Promise<Record<string, string>> {
  const fileBlob = await toFile(Buffer.from(content), name, { type: mimeType });
  ...
  const prompt = buildFilePrompt(formType, context, targetFields, locale, descriptions);
  ...
  try {
    ...
    return sanitize(parseJson(rawText), targetFields);
  } finally {
...
/** Suggests form field values from the already-known context alone. */
export async function suggestFieldsFromContext(
  formType: FormType,
  context: Record<string, string>,
  targetFields: string[],
  locale: Locale,
