...
function seedFields(c: Client): ClientFields {
  return {
    name: c.name,
    cui: c.cui ?? "",
    j_number: c.j_number ?? "",
...
export function EditClientDialog({ client, open, onClose }: Props) {
  const t = useTranslations("clients");
  const [clientType, setClientType] = useState<ClientType>(client.type);
  const [fields, setFields] = useState<ClientFields>(() => seedFields(client));
  ...
  const aiClass = (fieldKey: keyof ClientFields) =>
    cn(hasSuggestions && fields[fieldKey] ? "ring-1 ring-veltol-aqua/30" : "");
          ...
          <form action={action} className="mt-6 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                >
                  {CLIENT_TYPES.map((ct) => (
                    <option key={ct} value={ct} className="bg-veltol-deep">
            ...
            {clientType === "company" && (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.jNumber")}</Label>
            ...
            {clientType === "company" && (
              <div className="space-y-1.5">
                <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.legalRep")}</Label>
            ...
            {clientType === "person" && (
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                ...
                <div className="space-y-1.5">
                  <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.idSeries")}</Label>
                  <Input name="id_series" value={fields.id_series} onChange={setField("id_series")} maxLength={2} className={aiClass("id_series")} />
                </div>
                <div className="space-y-1.5">
                  <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.idNumber")}</Label>
            ...
            <div className="space-y-1.5">
              <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.regAddress")}</Label>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.phone")}</Label>
                <Input name="phone" value={fields.phone} onChange={setField("phone")} className={aiClass("phone")} />
              </div>
            </div>

            <div className="space-y-1.5">
              <Label className="mono-label text-[9px] text-veltol-fgMute">{t("fields.email")}</Label>
