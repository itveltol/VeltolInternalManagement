...
import { createCustomTaskAction } from "@/app/[locale]/(app)/projects/[id]/actions";
...
export function AddCustomTaskDialog({ projectId, teams, open, onClose }: Props) {
  ...
  const [state, action, pending] = useActionState(createCustomTaskAction, null);
        ...
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 overflow-y-auto max-h-[90dvh] rounded-xl border border-border bg-card p-5 shadow-2xl sm:p-8">
          ...
          <form action={action} className="mt-6 space-y-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-[11px] font-medium text-veltol-fgMute">{t("gantt.taskName")} *</Label>
              <Input
                name="name"
                required
                value={name}
            ...
            <div className="space-y-1.5">
              <Label className="text-[11px] font-medium text-veltol-fgMute">{t("gantt.taskPhase")}</Label>
              <Input
                name="phase"
                value={phase}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-[11px] font-medium text-veltol-fgMute">{t("gantt.planTotal")}</Label>
                <Input
                  name="plan_total"
                  type="number"
                  min="0"
              ...
              <div className="space-y-1.5">
                <Label className="text-[11px] font-medium text-veltol-fgMute">{t("gantt.plannedDays")}</Label>
                <Input
                  name="zile"
                  type="number"
                  min="0"
