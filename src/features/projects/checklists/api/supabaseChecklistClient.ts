...
export const createSupabaseChecklistClient = (supabase: SupabaseClient): ChecklistApiClient => ({
  ...
  async getChecklistRecordsForProjects(projectIds) {
  ...
  async upsertChecklistItem({ projectId, itemNumber, plan_total, zile, persons_allocated, units_per_person_day, notes }: UpsertChecklistItemPayload) {
    const { error } = await supabase
      .from("project_checklist_items")
      .upsert(
        { project_id: projectId, item_number: itemNumber, plan_total, zile, persons_allocated, units_per_person_day, notes },
