...
import { createBudgetLineAction } from "@/app/[locale]/(app)/projects/[id]/financiar-actions";
...
export function AddBudgetLineDialog({ projectId, categories, exchangeRate, open, onClose }: Props) {
  return (
...
function AddBudgetLineForm({ projectId, categories, exchangeRate, onClose }: Omit<Props, "open">) {
  const t = useTranslations("financiar");
  const [state, action, pending] = useActionState(createBudgetLineAction, null);
  ...
  const handleQty = useCallback((e: React.ChangeEvent<HTMLInputElement>) => setQty(e.target.value), []);
      ...
      <form action={action} className="mt-6 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input name="phase_no" type="number" min="0" step="1" />
          </div>
        </div>
