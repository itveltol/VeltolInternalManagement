-- Removes the progress_pct_manual escape hatch: Matrice completion is now
-- the sole source of truth for projects.progress_pct, with no way to pin a
-- hand-typed value. status_manual/projects.status is untouched — a separate
-- flag, out of scope for this change.
...
-- Backfill: correct any project that was left pinned to a stale manual value
-- immediately, rather than waiting for its next Matrice cell edit.
...
where proj.progress_pct_manual;

alter table public.projects drop column if exists progress_pct_manual;
